...
guidelines:
  do:
    - "Use a single navy primary as the clear next step."
    - "Start the label with a verb."
  dont:
    - "Stack several primaries so nothing stands out."
    - "Use a brand CTA inside the internal app."
---
...
import { Button, Icon } from "@ucm/ui";
...
The primary button shows a forward arrow by default (an inline SVG, no icon
dependency); pass `hideArrow` to remove it. `iconLeft` places a leading icon node.
...
| Android  | -         | -                       |
| iOS      | -         | -                       |
| Web      | <Icon name="check_circle" /> | `@ucm/ui` → `Button`    |
